...
describe('HittableList', () => {  
  ...
  const rayTowardsSphere1 = { origin: new Vec3(0, 0, 0), direction: new Vec3(0, 0, -1) };
  const rayTowardsSphere2 = { origin: new Vec3(0, 100, 0), direction: new Vec3(0, 0, -1) };
  const tMin = 0.001;
  const tMax = Infinity;
  ...
  test('should initialize empty (no hits)', () => {
    ...
    expect(list.hit(rayTowardsSphere1.origin, rayTowardsSphere1.direction, fullInterval.min, fullInterval.max)).toBeNull();
  });

  test('should add objects and allow hits', () => {
    // Add sphere1, check hit
    list.add(sphere1);
    expect(list.hit(rayTowardsSphere1.origin, rayTowardsSphere1.direction, fullInterval.min, fullInterval.max)).not.toBeNull();
    // Check that sphere2 isn't hit yet
    expect(list.hit(rayTowardsSphere2.origin, rayTowardsSphere2.direction, fullInterval.min, fullInterval.max)).toBeNull();

    // Add sphere2, check hit
    list.add(sphere2);
    expect(list.hit(rayTowardsSphere1.origin, rayTowardsSphere1.direction, fullInterval.min, fullInterval.max)).not.toBeNull(); // Sphere1 still hittable
    expect(list.hit(rayTowardsSphere2.origin, rayTowardsSphere2.direction, fullInterval.min, fullInterval.max)).not.toBeNull(); // Sphere2 now hittable
  });

  test('should clear objects (no hits)', () => {
    list.add(sphere1);
    expect(list.hit(rayTowardsSphere1.origin, rayTowardsSphere1.direction, fullInterval.min, fullInterval.max)).not.toBeNull(); // Verify it was added
    list.clear();
    expect(list.hit(rayTowardsSphere1.origin, rayTowardsSphere1.direction, fullInterval.min, fullInterval.max)).toBeNull(); // Should be null after clear
  });
  describe('hit', () => {
    ...
    const rayFromOrigin = { origin: new Vec3(0, 0, 0), direction: new Vec3(0, 0, -1) };
    // tMin/tMax defined in outer scope
    // fullInterval defined in outer scope

    test('should return null for an empty list', () => {
      const hitRec = list.hit(rayFromOrigin.origin, rayFromOrigin.direction, fullInterval.min, fullInterval.max);
    ...
    test('should return null if ray misses all objects', () => {
      list.add(sphere1Hit);
      list.add(sphere2Hit);
      const missingRay = { origin: new Vec3(5, 5, 0), direction: new Vec3(0, 0, -1) };
      const hitRec = list.hit(missingRay.origin, missingRay.direction, fullInterval.min, fullInterval.max);
      expect(hitRec).toBeNull();
    });
    ...
    test('should return the farther hit if closer object is outside tMax range (via closestSoFar)', () => {
      ...
      const hitRec = list.hit(rayFromOrigin.origin, rayFromOrigin.direction, tMin, 1.0);
    ...
    test('should return closer object\'s farther hit if it\'s closer than others', () => {
      ...
      const hitRec = list.hit(rayFromOrigin.origin, rayFromOrigin.direction, 1.0, tMax);
      expect(hitRec).not.toBeNull();
    ...
    test('should return null if all hits are outside [tMin, tMax]', () => {
      list.add(sphere1Hit); // Hits at t=0.5, 1.5
      list.add(sphere2Hit); // Hits at t=2.5, 3.5
      const hitRec = list.hit(rayFromOrigin.origin, rayFromOrigin.direction, 1.6, 2.4);
      expect(hitRec).toBeNull();
    });
  });
});